...
def compute_bdf_decile(input_bdf, type):
    ...
    input_bdf_by_decile = df_weighted_average_grouped(
        input_bdf_copy,
        groupe="decile_{}_niveau_vie".format(type),
        varlist=["rev_disponible", "niveau_de_vie_bdf", "ocde10"],
        weights="pond{}".format(type),
    )
...
def compute_erfs_decile(target_year, type, path):
    ...
    erfs_menage["decile_{}_niveau_vie".format(type)] = weighted_quantiles(
        erfs_menage["niveau_de_vie"],
        labels=np.arange(1, 11),
        weights=erfs_menage["pond{}".format(type)],
        return_quantiles=False,
    )
    erfs_menage_by_decile = df_weighted_average_grouped(
        erfs_menage,
        groupe="decile_{}_niveau_vie".format(type),
        varlist=["revdispm", "niveau_de_vie", "nb_uci"],
        weights="pond{}".format(type),
    )
