...
from openfisca_france_indirect_taxation.utils import (
    assets_directory,
    get_input_data_frame,
)
from openfisca_france_indirect_taxation.Correction_territoriale import (
    get_correction_territoriale,
)
...
def get_cn_aggregates(target_year):
    ...
    liste_13postes = ["poste_0{}".format(i) for i in range(1, 10)] + [
        "poste_10",
        "poste_11",
        "poste_12",
        "poste_13",
...
def get_inflators_cn_23_to_24():
    ...
    comptes_trim = pd.read_excel(
        os.path.join(comptes_trimestriels_folder_path, "t_conso_val.xls"),
        sheet_name="Niveaux",
        header=4,
    )
...
def get_inflators_by_year(rebuild=False, year_range=None, data_year=None):
    ...
    if rebuild is not False:
        ...
        writer_inflators = csv.writer(
            open(
                os.path.join(assets_directory, "inflateurs", "inflators_by_year.csv"),
                "w",
            )
        )
