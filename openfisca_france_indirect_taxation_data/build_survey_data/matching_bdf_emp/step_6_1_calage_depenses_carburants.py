...
def calage_depenses_from_distance(data_matched):
    ...
    for (decile, rur), group in grouped:
        # weighted averages
        avg_distance = (group["distance"] * group["pondmen"]).sum() / group[
            "pondmen"
        ].sum()
        avg_depenses = (group["poste_07_2_2_1"] * group["pondmen"]).sum() / group[
            "pondmen"
        ].sum()
        ...
        idx = (data_matched["niveau_vie_decile"] == decile) & (
            data_matched["rural"] == rur
        )
        data_matched.loc[idx, "depenses_carburants_corrigees_emp"] = (
            data_matched.loc[idx, "distance"] * factor
        )
        data_matched.loc[idx, "depenses_diesel_corrigees_emp"] = (
            data_matched.loc[idx, "distance_diesel"] * factor
        )
        data_matched.loc[idx, "depenses_essence_corrigees_emp"] = (
            data_matched.loc[idx, "distance_essence"] * factor
        )
