...
data_emp = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_emp", "data_matching_emp.csv"),
    sep=",",
    decimal=".",
)
...
data_matched_rank = pd.read_csv(
    os.path.join(assets_directory, "matching", "matching_emp", "data_matched_rank.csv"),
    sep=",",
    decimal=".",
)
...
for i, ax in enumerate(axes.flat):
    if i < len(urban_cat):
        cat = urban_cat[i]
        sns.kdeplot(
            data=data_emp.loc[data_emp[cat] == 1],
            x="distance",
            weights="pondmen",
            bw_adjust=0.7,
            ax=ax,
